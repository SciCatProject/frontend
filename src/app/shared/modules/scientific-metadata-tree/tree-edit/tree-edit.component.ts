...
export class TreeEditComponent extends TreeBaseComponent
  ...
  convertDataTreeToObject(tree: TreeNode[]): { [key: string]: any } {
    return tree.reduce((accumulator: Record<string, unknown>, node) => {
  ...
  onSave(data: InputData | null) {
    if (data) {
      ...
      if (nestedNode) {
        ...
        if (mappedNestedNode) {
          const parentNode = this.getNestedParent(mappedNestedNode);
          if (parentNode) {
  ...
  addNewNode(parentNode: FlatNodeEdit | null) {
    if (parentNode) {
      ...
      this.historyManager.add({
        undo: () => {
          if (parentNode.children.length === 0) {
            ...
            const mappedNestedParentNode = this.nestNodeMap.get(parentNode);
            if (mappedNestedParentNode) {
              const grandfatherNode = this.getNestedParent(
                mappedNestedParentNode
              );
              if (grandfatherNode) {
  ...
  openObjectCreationDialog(node: FlatNodeEdit | null): void {
