...
export class ConfigurableActionComponent implements OnInit, OnChanges {
  ...
  private variableHandler(selector: string): unknown {
    ...
    const fieldMatch = selector.match(/\[(\w+)\]/);
    if (fieldMatch) return this.fieldMatch(selector, fieldMatch);
  ...
  private fieldMatch(selector: string, fieldMatch: RegExpMatchArray): unknown {
    ...
    if (selector.startsWith("#DatasetsField")) return _.map(datasets, field);
    const indexMatch = selector.match(/\d+/);
    return _.get(datasets, `[${indexMatch?.[0]}].${field}`);
  }
  ...
  private authorizationHandlers(definition: string): string {
    ...
    return tokens[definition] ?? definition;
  }
  ...
  private typeXhr() {
    ...
    Object.entries(this.actionConfig.headers || {}).forEach(([k, v]) => {
      headers[k] = v.startsWith("#")
        ? this.authorizationHandlers(v)
        : String(this.resolve(v));
    });
  ...
  private typeForm() {
    ...
    Object.entries(this.actionConfig.inputs || {}).forEach(([input, def]) => {
      // Check for auth tokens specifically in form inputs
      const value = def.startsWith("#")
        ? this.authorizationHandlers(def)
        : this.resolve(def);
