...
export class PolicyApi extends BaseLoopBackApi {
  ...
  /**
   * responsible for updating multiple records on the Policy model simultaneously and uses ownerGroup to identify those records
   *
   * @param {object} data Request data.
   *
   *  - `ownerGroupList` – `{string}` - comma seperated string of owner groups to update e.g. "p14159, p24959"
   *
   *  - `data` – `{object}` - 
   *
   * @returns {object} An empty reference that will be
