...
export class ParamsService {
  constructor(
    ...
    this.store.pipe(select(getViewMode)).subscribe(currentMode => {
      if (currentMode) {
        this.route.queryParams.pipe(take(1)).subscribe(params => {
          const newParams =
            "args" in params
              ? rison.decode(params["args"])
              : dStore.initialDatasetState.filters;
          newParams["mode"] = currentMode;
          if (window.location.pathname.indexOf("datasets") !== -1) {
            this.router.navigate(["/datasets"], {
              queryParams: { args: rison.encode(newParams) }
            });
          }
        });
      }
