...
export class EndOfShiftComponent implements OnInit {
  ...
  constructor(private cds: DatasetService, private ds: RawDatasetApi,
              private ps: ProposalApi, private store: Store<any>) {
    this.ps.find({limit : 10}).subscribe(proposals => {
      console.log(proposals);
    });
    this.cds.searchDatasetsObservable({'order' : 'creationTime DESC'})
        .subscribe(results => { this.datasets = <Array<RawDataset>>results; },
                   error => {
                     this.store.dispatch({
