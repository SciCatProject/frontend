...
@Component({
    ...
    <div class="anyof-group">
      ...
      <mat-radio-group [(ngModel)]="selectedOption" (change)="onOptionChange()">
        <mat-radio-button *ngFor="let option of options" [value]="option" style="margin-right: 10px;">
          {{option}}
        </mat-radio-button>
      </mat-radio-group>
      <div style="margin-top: 20px;" *ngIf="selectedAnyOption && selectedOption !== 'null'">
        <jsonforms [schema]="selectedAnyOption" [data]="passedProps.data" [renderers]="defaultRenderer" (dataChange)=onInnerJsonFormsChange($event)></jsonforms>
      </div>
    </div>
  `
})
export class OneOfRenderer extends JsonFormsControl {
    ...
    public onOptionChange() {
        this.selectedAnyOption = (this.passedProps.schema.anyOf as any).find((option: any) => option.title === this.selectedOption || option.type === this.selectedOption || JSON.stringify(option) === this.selectedOption);
    }
